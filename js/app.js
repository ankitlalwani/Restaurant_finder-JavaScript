class ZOMATO {
    constructor(){
        this.api = "a93d8816c432562e232060327a68bdcd";
        this.header = {
            method: "GET",
            headers: {
                "user-key": this.api,
...
getRestaurants(restaurants){
    ...
    if(restaurants.length === 0){
        this.showFeedback(`no such categories exists in the selected city`)
    }
    else{
        this.restaurantList.innerHTML = '';
        restaurants.forEach(restaurant =>{
            const {thumb: img, name, location:{address}, user_rating: {aggregate_rating}, cuisines: cuisines,
                 average_cost_for_two:cost, menu_url,url} = restaurant.restaurant;
            if(img !== ''){
